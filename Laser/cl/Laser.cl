...
__constant unsigned int SAMPLES = 16;
__constant unsigned int MAX_DEPTH = 3;
...
float3 trace(struct Ray* primaryRay, __global float3* vertices, __global struct Triangle* triangles,
	unsigned int n_Triangles, __global struct Material* materials, __global mat4* transforms,
	__global struct RenderStats* renderStats, unsigned int* seed0, unsigned int* seed1)
{
...
__kernel void Laser(__global float3* output, unsigned int imageWidth, unsigned int imageHeight,
	...
	__global mat4* transforms, __global struct RenderStats* renderStats,
	...
	unsigned int tileWidth, unsigned int tileHeight)
{
	...
	for (int i = 0; i < SAMPLES; i++)
		color += trace(&primaryRay, vertices, triangles, n_Triangles, materials, transforms, renderStats, &seed0, &seed1) * invSamples;
