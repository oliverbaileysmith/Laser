...
void reflectDiffuse(Ray* ray, Intersection* isect, uint* seed)
{
	...
	ray->dir = normalize(u * cos(rand1)*rand2s + v*sin(rand1)*rand2s + w*sqrt(1.0f - rand2));
...
void refract(Ray* ray, Intersection* isect, float ratioIOR, bool frontFace)
{
	...
	float3 rOutParallel = -sqrt(fabs(1.0f - pow(length(rOutPerp), 2.0f))) * isect->N;
...
float3 computeSmoothNormal(Intersection* isect, float3* v0n, float3* v1n, float3* v2n, mat4* m)
{	
	float3 normal;
